function getShipmentFromAmazon(config, query_type, marketplace_id, shipment_status_list, shipment_id_list, last_updated_after, last_updated_before, next_token){
  try {

         let shipmentsResponse = getShipmentsAPI(config, query_type, marketplace_id, shipment_status_list, shipment_id_list, last_updated_after, last_updated_before, next_token);
...
Logger.log(shipmentResponse)
          while(_nextToken) {
            ...
            if (_nextToken) {
              ...
              let nextShipmentsResponseData = getShipmentsAPI(config, next_query_type, marketplace_id, null, null, null, null, _nextToken);
        ...
        Logger.log('Error while getting shipments from amazon :' + e);
        throw new Error('Error while getting inbound shipment API:' +e);
    }
}


function getShipmentItemsFromAmazon(config, query_type, marketplace_id, last_updated_after, last_updated_before, next_token){
  try {
          let shipmentsResponse = getShipmentitemsAPI(config, query_type, marketplace_id, last_updated_after, last_updated_before, next_token);
          if(shipmentsResponse) {
            ...
            if(shipmentItems) {
              ...
              while(_nextToken) {
                ...
                if(_nextToken) {
                  var next_query_type = 'NEXT_TOKEN';
                  let nextShipmentsResponseData = getShipmentitemsAPI(config, next_query_type, marketplace_id, last_updated_after, last_updated_before, _nextToken)
                  var nextData = JSON.parse(nextShipmentsResponseData);
                  var nextResponses = nextData.payload;
                  var _nextToken = nextResponses.NextToken;
...
function getShipmentItemsByShipmentIdFromAmazon(shipment_id, marketplace_id) {
  try {
...
function getShipmentsAPI(config, query_type, marketplace_id, shipment_status_list, shipment_id_list, last_updated_after, last_updated_before, next_token) {
...
function getShipmentitemsAPI(config, query_type, marketplace_id, last_updated_after, last_updated_before, next_token) {
   try {
...
function getShipmentItemsByShipmentIdAPI(shipment_id, marketplace_id) {
   try {
        var config = JSON.parse(loadCredentials());
        let _role_credentials = getSessionCredential(config);
        var access_token = getAccessToken(config);
...
function getTransportDetailsAPI(config, shipment_id, marketplace_id) {
   try {
        //var config = JSON.parse(loadCredentials());
        let _role_credentials = getSessionCredential(config);
        var access_token = getAccessToken(config);
